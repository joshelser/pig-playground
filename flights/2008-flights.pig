...
flight_data = FOREACH flight_data GENERATE CONCAT(FORMAT('%04d-%02d-%02d', year, month, dayofmonth), CONCAT('_', CONCAT(uniquecarrier, CONCAT('_', (chararray)flightnum)))) as rowkey,
...
STORE flight_data into 'accumulo://flights?instance=accumulo1.4&user=root&password=secret&zookeepers=localhost' using org.apache.accumulo.pig.AccumuloStorage();

-- delays = FILTER delays BY total_delay > 0;

-- delays = ORDER delays BY total_delay;
